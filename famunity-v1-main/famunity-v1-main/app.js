...
searchUserBtn.addEventListener('click', async () => {
  ...
  try {
    ...
    const emailQuery = query(collection(db, 'users'), where('email', '==', searchUserInput.value.trim()));
    const emailSnap = await getDocs(emailQuery);
    
    // Se não encontrar por email, buscar todos os usuários e filtrar por nome
    let results = [];
    
    if (!emailSnap.empty) {
      emailSnap.forEach(doc => results.push(doc));
    } else {
      // Se email não encontrou, buscar em todos os usuários
      const allUsersQuery = query(collection(db, 'users'));
      const allUsersSnap = await getDocs(allUsersQuery);
      
      allUsersSnap.forEach(docSnap => {
        const userData = docSnap.data();
        // Verificar se o nome ou email contém o termo de busca
